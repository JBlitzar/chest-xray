...
class ChestXrayDataset(Dataset):
    def __init__(self, root_dir, transform=None, train=False):
        ...
        if os.path.exists(os.path.join(root_dir, "cache")):
            self.img_cache = torch.load(
                os.path.join(root_dir, "cache", "img_cache.pt"), weights_only=True
            )
            self.target_cache = torch.load(
                os.path.join(root_dir, "cache", "target_cache.pt"), weights_only=True
            )
        else:
    ...
    def make_cache(self):
        ...
        for img_path in tqdm.tqdm(self.file_list):
            ...
            image = (
                torch.as_tensor(
                    np.array(Image.open(img_path).convert("L"), dtype=np.uint8),
                    dtype=torch.float32,
                ).unsqueeze(0)
                / 255.0
            )
            ...
            self.target_cache = torch.cat(
                (self.target_cache, torch.tensor([target])), dim=0
            )

        torch.save(self.img_cache, os.path.join(self.root, "cache", "img_cache.pt"))
        torch.save(
            self.target_cache, os.path.join(self.root, "cache", "target_cache.pt")
        )
